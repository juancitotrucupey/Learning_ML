...
class RBM:
    ...
    def __init__(self, n_visible=784, n_hidden=500, momentum=False):
        ...
        self.w = tf.Variable(tf.truncated_normal([n_visible, n_hidden], stddev=0.1), name='w') 
        self.hb = tf.Variable(tf.constant(0.1, shape=[n_hidden]), name='hb') 
        self.vb = tf.Variable(tf.constant(0.1, shape=[n_visible]), name='vb') 
